...
export function ServiceProvider({ adminInfo }) {
    ...
    const fetchCountServiceProvider = async () => {
        try {
            const res = await axios.get(`http://localhost:9000/api/service-provider/count`, {
                headers: {
                    Authorization: 'Bearer ' + adminInfo.token,
                },
            })
    ...
    return (
        <>
            <div className="col-xl-3 col-md-6">
                <div className="card bg-primary text-white mb-4">
                    <div className="card-body">
                        Service Provider  <span>Count : {countServiceProvider}</span>
                    </div>
                </div>
            </div>
        </>
