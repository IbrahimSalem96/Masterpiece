...
export default function TableSort() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:9000/api/services/rental/', {
                headers: {
    ...
    const handleChangeCondition = async (id) => {
        try {
            const response = await axios.put(`http://localhost:9000/api/services/rental/status/${id}`, {}, {
                headers: {
                    Authorization: 'Bearer ' + adminInfo.token,
                },
            });
    ...
    const rows = records.map((row) => (
        <tr key={row._id}>
            ...
            <td>{row.bookDate}</td>
            <td>{row.location}</td>
            <td>{row.priceOneDay}</td>
            <td>{row.vehicleCompany}</td>
            <td>{row.nameVehicle}</td>
    ...
    return (
        <Container size="xl" style={{ paddingTop: '20px' }}>
            ...
            <CreateRental adminInfo={adminInfo} fetchData={fetchData} />
