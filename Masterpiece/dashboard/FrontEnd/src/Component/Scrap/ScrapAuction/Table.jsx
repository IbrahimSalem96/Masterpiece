...
export default function TableSort() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:9000/api/scrap/auction-service/', {
                headers: {
    ...
    //Active Action 
    const activeAction = async (actionId) => {
        try {
            const response = await axios.get(`http://localhost:9000/api/scrap/next-auction/active/${actionId}`, {
                headers: {
                    Authorization: 'Bearer ' + adminInfo.token,
                },
            });
    ...
    const rows = records.map((row) => (
        <tr key={row._id}>
            ...
            <td>{row.date}</td>
            <td>{row.time}</td>
            <td><img src={row.image.url} alt='img scrap' width='70' height='70' /></td>
            <td>{row.details}</td>
            <td>{row.active ? <button type="button" className="btn btn-danger w-100" onClick={() => activeAction(row._id)} >Inactive</button>
                : <button type="button" className="btn btn-success w-100" onClick={() => activeAction(row._id)}>Active</button>
            }</td>
            <td>
                <button type="button" className="btn btn-success me-2 mb-2" data-bs-toggle="modal" onClick={() => setEditId(row._id)} data-bs-target="#edit">
                    <i className="fa-solid fa-pen-to-square"></i>
                </button>
                <button type="button" className="btn btn-danger  mb-2" onClick={() => setDeleteId(row._id)} data-bs-toggle="modal" data-bs-target="#delete">
