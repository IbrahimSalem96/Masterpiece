...
import CreateBeforeBuying from './CreateBeforeBuying'
...
export default function TableSort() {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:9000/api/services/before-buying/', {
                headers: {
    ...
    //Change Condition
    const handleChangeCondition = async (id) => {
        try {
            const response = await axios.put(`http://localhost:9000/api/services/before-buying/status/${id}`, {}, {
                headers: {
                    Authorization: 'Bearer ' + adminInfo.token,
                },
            });
    ...
    const rows = records.map((row) => (
        <tr key={row._id}>
            ...
            <td>{row.time}</td>
            <td>{row.type}</td>
            <td>{row.vehicleCompany}</td>
            <td>{row.modelVehicle}</td>
            <td>{row.location}</td>
            <td>{row.fuelType}</td>
            <td>{row.details}</td>
            <td>
                {row.condition ? (
                    <button type="button" className="btn btn-success me-2" onClick={() => handleChangeCondition(row._id)} >
                        True
                    </button>
                ) : (
                    <button type="button" className="btn btn-danger" onClick={() => handleChangeCondition(row._id)}>
                        False
                    </button>
                )}
            </td>
    ...
    return (
        <Container size="xl" style={{ paddingTop: '20px' }}>
            ...
            <CreateBeforeBuying adminInfo={adminInfo} fetchData={fetchData} />
            ...
            <ScrollArea>
                ...
                />
                <Table horizontalSpacing="md" verticalSpacing="xs" w={1400} sx={{ tableLayout: 'scroll' }}>
                    <thead>
                        <tr>
                            <th>
                                Name
