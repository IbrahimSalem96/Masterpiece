...
function CreateServiceProviders({ adminInfo, fetchData }) {
    ...
    const handleEdit = async () => {
        ...
        if (!fields.DateSelection) {
            newErrorMessages.DateSelection = 'Date is required';
        }

        if (!fields.TimeSelection) {
            newErrorMessages.TimeSelection = 'Time is required';
        }
        ...
        try {
            const response = await axios.post(`http://localhost:9000/api/maintenance/`, fields, {
                headers: {
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <form>
                            ...
                            <div className="mb-3">
                                <label htmlFor="exampleInputDateSelection" className="form-label">Date</label>
                                <input
                                    type="date"
                                    className="form-control"
                                    id="exampleInputDateSelection"
                                    aria-describedby="DateSelectionHelp"
                                    value={fields.DateSelection}
                                    onChange={(e) => handleFieldChange('DateSelection', e.target.value)}
                                />
                                {errorMessages.DateSelection && <div className="text-danger">{errorMessages.DateSelection}</div>}
                            </div>

                            <div className="mb-3">
                                <label htmlFor="exampleInputTimeSelection" className="form-label">Time</label>
                                <input
                                    type="time"
                                    className="form-control"
                                    id="exampleInputTimeSelection"
                                    aria-describedby="TimeSelectionHelp"
                                    value={fields.TimeSelection}
                                    onChange={(e) => handleFieldChange('TimeSelection', e.target.value)}
                                />
                                {errorMessages.TimeSelection && <div className="text-danger">{errorMessages.TimeSelection}</div>}
                            </div>
