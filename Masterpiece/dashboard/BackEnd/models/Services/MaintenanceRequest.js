...
const maintenanceRequestSchema = new Schema({
    ...
    requist: {
        ...
        required: true,
        trim: true,
        enum: ['Flat Tire', 'Engine Heat', 'Battery Jump', 'Key lock', 'Other service']
    },
    vehicleCompany: {
        type: String,
        required: true,
...
const MaintenanceRequest = mongoose.model("maintenance_request", maintenanceRequestSchema)


//Validate Created Maintenance Request
function ValidateCreatedMaintenanceRequest(obj) {
    const Schema = Joi.object({
        type: Joi.string().valid('Cars', 'Motorcycles', 'Buses', 'Trucks', 'Machines').trim().required(),
        requist: Joi.string().valid('Flat Tire', 'Engine Heat', 'Battery Jump', 'Key lock', 'Other service').trim().required(),
...
function ValidateUpdateMaintenanceRequest(obj) {
    const Schema = Joi.object({
        type: Joi.string().valid('Cars', 'Motorcycles', 'Buses', 'Trucks', 'Machines').trim(),
        requist: Joi.string().valid('Flat Tire', 'Engine Heat', 'Battery Jump', 'Key lock', 'Other service').trim(),
...
module.exports = { MaintenanceRequest, ValidateCreatedMaintenanceRequest, ValidateUpdateMaintenanceRequest }